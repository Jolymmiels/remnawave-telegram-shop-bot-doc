---
title: Установка
description: Пошаговая инструкция по установке приватной версии Remnawave Telegram Shop Bot
---

import { Steps, Tabs, TabItem } from '@astrojs/starlight/components';

:::caution[Требуется бекап]
Перед любыми действиями ниже обязательно **сделайте бекап базы данных и конфигурационных файлов**.
:::

## Требования

- Linux сервер или VPS
- Установленные Docker и Docker Compose
- Доменное имя с SSL-сертификатом
- Реверс-прокси (Caddy или Nginx)
- Установленная и настроенная панель Remnawave
- Файл Docker образа приватной версии (`.tar`)

---

## Варианты установки

Выберите способ установки в зависимости от вашей конфигурации:

<Tabs>
  <TabItem label="На сервере с Remnawave">
    
### Установка рядом с панелью Remnawave

Используйте этот способ, если бот будет работать на **том же сервере**, что и панель Remnawave. Бот подключается к Remnawave через внутреннюю Docker-сеть.

<Steps>

1. **Создайте директорию проекта**
   ```bash
   mkdir -p /opt/private-remnawave-telegram-shop-bot
   cd /opt/private-remnawave-telegram-shop-bot
   ```

2. **Загрузите Docker образ**
   
   Скачайте файл образа из приватного канала.
   
   ```bash
   docker load -i private-remnawave-telegram-shop-bot-<VERSION>.tar
   ```

3. **Создайте необходимые директории**
   ```bash
   mkdir -p uploads translations
   sudo chmod -R 777 uploads translations
   ```

4. **Скачайте файлы переводов**
   
   Скачайте `en.json` и `ru.json` из приватного канала и поместите в папку `translations`:
   ```bash
   # Скопируйте скачанные файлы в директорию translations
   cp en.json ru.json ./translations/
   ```

5. **Создайте конфигурацию Docker Compose**
   ```bash
   nano compose.yaml
   ```
   
   ```yaml
   services:
     bot:
       image: private-remnawave-telegram-shop-bot:<VERSION>
       env_file:
         - .env
       ports:
         - "127.0.0.1:9912:8080"
       depends_on:
         db:
           condition: service_healthy
       volumes:
         - ./translations:/translations
         - ./uploads:/uploads
       restart: unless-stopped
       networks:
         - remnawave-network

     db:
       image: postgres:17-alpine
       container_name: remnawave-telegram-shop-db
       hostname: remnawave-telegram-shop-db
       restart: unless-stopped
       env_file:
         - .env
       environment:
         - POSTGRES_USER=${POSTGRES_USER}
         - POSTGRES_PASSWORD=${POSTGRES_PASSWORD}
         - POSTGRES_DB=${POSTGRES_DB}
         - TZ=UTC
       ports:
         - "127.0.0.1:9911:5432"
       volumes:
         - remnawave-telegram-shop-db-data:/var/lib/postgresql/data
       healthcheck:
         test: ["CMD-SHELL", "pg_isready -U ${POSTGRES_USER} -d ${POSTGRES_DB}"]
         interval: 3s
         timeout: 10s
         retries: 3
       networks:
         - remnawave-network

   volumes:
     remnawave-telegram-shop-db-data:
       driver: local
       external: false
       name: remnawave-telegram-shop-db-data

   networks:
     remnawave-network:
       name: remnawave-network
       driver: bridge
       external: true
   ```
   
   :::note
   Сеть `remnawave-network` создаётся панелью Remnawave. Параметр `external: true` подключает к существующей сети.
   :::

6. **Создайте файл окружения**
   ```bash
   nano .env
   ```
   
   ```bash
   # Подключение к Remnawave (внутренняя Docker-сеть)
   REMNAWAVE_URL=http://remnawave:3000
   REMNAWAVE_TOKEN=ваш_api_токен_remnawave

   # Telegram
   TELEGRAM_TOKEN=ваш_токен_telegram_бота
   ADMIN_TELEGRAM_ID=123456789,987654321

   # База данных
   DATABASE_URL=postgres://postgres:postgres@db:5432/postgres?sslmode=disable
   POSTGRES_USER=postgres
   POSTGRES_PASSWORD=postgres
   POSTGRES_DB=postgres

   # URL админ-панели (требуется HTTPS)
   BOT_ADMIN_URL=https://bot.example.com
   ```
   
   :::tip
   `REMNAWAVE_URL=http://remnawave:3000` использует имя контейнера из Docker-сети Remnawave.
   :::

7. **Запустите бота**
   ```bash
   docker compose up -d
   ```

</Steps>

  </TabItem>
  <TabItem label="На отдельном сервере">

### Установка на отдельном сервере

Используйте этот способ, если бот будет работать на **другом сервере**, отличном от панели Remnawave. Бот подключается к Remnawave через внешний HTTPS URL.

<Steps>

1. **Создайте директорию проекта**
   ```bash
   mkdir -p /opt/private-remnawave-telegram-shop-bot
   cd /opt/private-remnawave-telegram-shop-bot
   ```

2. **Загрузите Docker образ**
   
   Скачайте файл образа из приватного канала.
   
   ```bash
   docker load -i private-remnawave-telegram-shop-bot-<VERSION>.tar
   ```

3. **Создайте необходимые директории**
   ```bash
   mkdir -p uploads translations
   sudo chmod -R 777 uploads translations
   ```

4. **Скачайте файлы переводов**
   
   Скачайте `en.json` и `ru.json` из приватного канала и поместите в папку `translations`:
   ```bash
   # Скопируйте скачанные файлы в директорию translations
   cp en.json ru.json ./translations/
   ```

5. **Создайте конфигурацию Docker Compose**
   ```bash
   nano compose.yaml
   ```
   
   ```yaml
   services:
     bot:
       image: private-remnawave-telegram-shop-bot:<VERSION>
       env_file:
         - .env
       ports:
         - "127.0.0.1:9912:8080"
       depends_on:
         db:
           condition: service_healthy
       volumes:
         - ./translations:/translations
         - ./uploads:/uploads
       restart: unless-stopped

     db:
       image: postgres:17-alpine
       container_name: remnawave-telegram-shop-db
       hostname: remnawave-telegram-shop-db
       restart: unless-stopped
       env_file:
         - .env
       environment:
         - POSTGRES_USER=${POSTGRES_USER}
         - POSTGRES_PASSWORD=${POSTGRES_PASSWORD}
         - POSTGRES_DB=${POSTGRES_DB}
         - TZ=UTC
       ports:
         - "127.0.0.1:9911:5432"
       volumes:
         - remnawave-telegram-shop-db-data:/var/lib/postgresql/data
       healthcheck:
         test: ["CMD-SHELL", "pg_isready -U ${POSTGRES_USER} -d ${POSTGRES_DB}"]
         interval: 3s
         timeout: 10s
         retries: 3

   volumes:
     remnawave-telegram-shop-db-data:
       driver: local
       external: false
       name: remnawave-telegram-shop-db-data
   ```

6. **Создайте файл окружения**
   ```bash
   nano .env
   ```
   
   ```bash
   # Подключение к Remnawave (внешний HTTPS URL)
   REMNAWAVE_URL=https://panel.example.com
   REMNAWAVE_TOKEN=ваш_api_токен_remnawave

   # Telegram
   TELEGRAM_TOKEN=ваш_токен_telegram_бота
   ADMIN_TELEGRAM_ID=123456789,987654321

   # База данных
   DATABASE_URL=postgres://postgres:postgres@db:5432/postgres?sslmode=disable
   POSTGRES_USER=postgres
   POSTGRES_PASSWORD=postgres
   POSTGRES_DB=postgres

   # URL админ-панели (требуется HTTPS)
   BOT_ADMIN_URL=https://bot.example.com
   ```
   
   :::caution
   `REMNAWAVE_URL` должен быть **внешним HTTPS URL** вашей панели Remnawave (например, `https://panel.example.com`).
   :::

7. **Запустите бота**
   ```bash
   docker compose up -d
   ```

</Steps>

  </TabItem>
</Tabs>

---

## Настройка реверс-прокси

<Tabs>
  <TabItem label="Caddy">
    Добавьте в Caddyfile:
    ```txt
    bot.example.com {
        reverse_proxy 127.0.0.1:9912
    }
    ```
    
    Перезапустите Caddy:
    ```bash
    sudo systemctl reload caddy
    ```
  </TabItem>
  <TabItem label="Nginx">
    Создайте конфиг:
    ```bash
    sudo nano /etc/nginx/sites-available/bot.example.com
    ```
    
    ```nginx
    server {
        listen 80;
        server_name bot.example.com;
        return 301 https://$server_name$request_uri;
    }

    server {
        listen 443 ssl http2;
        server_name bot.example.com;

        ssl_certificate /etc/letsencrypt/live/bot.example.com/fullchain.pem;
        ssl_certificate_key /etc/letsencrypt/live/bot.example.com/privkey.pem;

        location / {
            proxy_pass http://127.0.0.1:9912;
            proxy_http_version 1.1;
            proxy_set_header Upgrade $http_upgrade;
            proxy_set_header Connection "upgrade";
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;
            
            # Настройки загрузки файлов
            client_max_body_size 50M;
        }
    }
    ```
    
    Активируйте и перезапустите:
    ```bash
    sudo ln -s /etc/nginx/sites-available/bot.example.com /etc/nginx/sites-enabled/
    sudo nginx -t
    sudo systemctl reload nginx
    ```
  </TabItem>
</Tabs>

---

## Проверка установки

```bash
docker compose logs -f bot
```

---

## Обновление

Для обновления до новой версии:

<Steps>

1. **Скачайте новый образ**
   
   Скачайте новый файл образа из приватного канала.

2. **Загрузите новый образ**
   ```bash
   docker load -i private-remnawave-telegram-shop-bot-<VERSION>.tar
   ```

3. **Обновите версию в compose.yaml**
   
   Измените версию образа:
   ```yaml
   image: private-remnawave-telegram-shop-bot:<VERSION>
   ```

4. **Перезапустите бота**
   ```bash
   docker compose up -d
   ```

</Steps>

---

## Обновление с публичной версии

Если у вас уже установлена публичная версия:

<Steps>

1. **Перейдите в директорию бота**
   ```bash
   cd /opt/remnawave-telegram-shop
   ```
   :::note
   Замените на ваш фактический путь установки бота, если он отличается.
   :::

2. **Сделайте бекап базы данных**
   ```bash
   docker compose exec db pg_dump -U postgres postgres > backup.sql
   ```

3. **Остановите текущего бота**
   ```bash
   docker compose down
   ```

4. **Загрузите новый образ**
   ```bash
   docker load -i private-remnawave-telegram-shop-bot-<VERSION>.tar
   ```

5. **Создайте директорию uploads**
   ```bash
   mkdir -p uploads
   sudo chmod -R 777 uploads
   ```

6. **Обновите docker-compose.yaml**
   
   Измените образ и добавьте порты:
   ```yaml
   services:
     bot:
       image: private-remnawave-telegram-shop-bot:<VERSION>
       env_file:
         - .env
       ports:
         - "127.0.0.1:12345:8080"
       depends_on:
         db:
           condition: service_healthy
       volumes:
         - ./translations:/translations
         - ./uploads:/uploads
   ```

7. **Добавьте URL админки в .env**
   ```bash
   echo "BOT_ADMIN_URL=https://bot.example.com" >> .env
   ```

8. **Настройте реверс-прокси**
   
   Направьте `bot.example.com` на `127.0.0.1:12345` (см. примеры выше)

9. **Запустите бота**
   ```bash
   docker compose up -d
   ```

</Steps>

---

## Справка по портам

| Сервис | Внутренний порт | Рекомендуемый внешний |
|--------|-----------------|----------------------|
| Bot HTTP | 8080 | 9912 или 12345 |
| PostgreSQL | 5432 | 9911 |

---

## После установки

После установки:

1. Откройте `https://bot.example.com` в Telegram Mini App
2. Войдите с вашим админским Telegram-аккаунтом
3. Настройте параметры в админ-панели:
   - Платёжные системы
   - Тарифные планы
   - Брендинг
   - Роли и разрешения

Все остальные настройки управляются через **UI админ-панели**.

---

## Защита Mini App от ошибок загрузки

При использовании SPA-приложений (например, Mini App) браузер может кешировать старые версии HTML-файлов. После обновления приложения пользователи могут увидеть ошибки загрузки, так как браузер пытается загрузить устаревшие ресурсы.

Решение — настроить кеширование на реверс-прокси:
- HTML-файлы: **отключить кеширование** (всегда загружать актуальную версию)
- Статические ресурсы (`/assets/`): **включить долгосрочное кеширование** (файлы имеют уникальные хеши в именах)

<Tabs>
  <TabItem label="Caddy">
    ```txt
    @mini-app host mini-app.example.com

    handle @mini-app {
        @html path *.html
        header @html Cache-Control "no-cache, no-store, must-revalidate"
        
        @assets path /assets/*
        header @assets Cache-Control "public, max-age=31536000, immutable"
        
        reverse_proxy 127.0.0.1:9912
    }
    ```
  </TabItem>
  <TabItem label="Nginx">
    ```nginx
    server {
        listen 80;
        server_name mini-app.example.com;

        location ~* \.html$ {
            add_header Cache-Control "no-cache, no-store, must-revalidate" always;
            proxy_pass http://127.0.0.1:9912;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;
        }

        location /assets/ {
            add_header Cache-Control "public, max-age=31536000, immutable" always;
            proxy_pass http://127.0.0.1:9912;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;
        }

        location / {
            proxy_pass http://127.0.0.1:9912;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;
        }
    }
    ```
  </TabItem>
</Tabs>

:::tip
Замените `mini-app.example.com` на ваш домен Mini App.
:::

---

## Устранение неполадок

### Админ-панель не загружается
- Проверьте правильность `BOT_ADMIN_URL` в `.env`
- Проверьте конфигурацию реверс-прокси
- Убедитесь, что SSL-сертификат валиден

### Ошибка подключения к базе данных
- Проверьте статус PostgreSQL контейнера: `docker compose ps`
- Проверьте формат `DATABASE_URL` в `.env`
- Убедитесь в сетевой связности между контейнерами

### Ошибка прав доступа для uploads
```bash
sudo chmod -R 777 /opt/private-remnawave-telegram-shop-bot/uploads
```

---

## Полезные советы

### Добавление кнопки меню в Telegram

Чтобы добавить кнопку "Open" в Telegram, которая открывает личный кабинет вместо набора команд:

1. Откройте [@BotFather](https://t.me/BotFather)
2. Выберите вашего бота
3. Перейдите в **Bot Settings** → **Menu Button**
4. Укажите URL вашего дашборда:
   ```
   https://bot.example.com/#/my-dashboard
   ```

Это добавит кнопку в чате Telegram, которая открывает Mini App с личным кабинетом пользователя.